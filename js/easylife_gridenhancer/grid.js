...
Easylife_GridEnhancer.Grid = Class.create();
Easylife_GridEnhancer.Grid.prototype = {
            ...
            for (var j in positionOptions){
                if (positionOptions.hasOwnProperty(j)){
                    if (j != this.fields[i].getCurrentValue()){
                        this.fields[i].positionElement.options.add(new Option(positionOptions[j].label, j, (j == currentPosition) ? 'selected="selected' : ''));
                    }
                }
            }
    ...
    addField:function(obj){
        ...
        var field = new Easylife_GridEnhancer.Field(
        ...
        if (obj == false){
            this.reloadFields();
            if (this.noticeAt != 0 && this.fields.length >= this.noticeAt && this.noticed == false){
                this.noticed = true;
    ...
    checkAvailability: function(){
        var that = this;
        $$(this.config.add_field_identifier).each(function(elem){
            var notValid = that.fields.length >= that.allOptions.length - 1;
            $(elem).disabled = notValid;
            if (notValid){
...
Easylife_GridEnhancer.Field = Class.create();
Easylife_GridEnhancer.Field.prototype = {
